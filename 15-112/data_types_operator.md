...
如果需要复制已有 `list` ，且与之前的 `list` 进行区分，推荐使用 `target_list.copy() or b = list(target_list)`

同一个 `list` 对象可以赋予不同的变量，这些变量最终指向的都是同一个 `list` 数据，所以修改其中一个变量，就会同步修改指向这个 `list` 的其他变量。

##### tuple

`tuple` 虽然宣称的是不会变化，但是如果 `tuple` 引用的对象实际是可修改的话，比如：
...
其实 `set` 其实存储了一个所有元素的 `hash map` ，所以查询才快且元素具有唯一性且无法放 list
